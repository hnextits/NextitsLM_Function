import requests, re, os, traceback, argparse, hashlib
...
def crawl_natenews_page(url: str, output_dir: str = "./"):
    """
    (최종 안정화 버전)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='네이트 뉴스 페이지를 크롤링하여 마크다운으로 저장합니다.')
    parser.add_argument('url', nargs='?', default="", help='크롤링할 네이트 뉴스 URL')
    ...
    if not url:
        url = "https://sports.news.nate.com/view/20251028n02637"
        if not output_dir:
            output_dir = "./Crawling_data"
    
    if 'nate' not in url:
        print("[ERROR] 유효한 네이트 뉴스 문서 URL이 아닙니다.")
        return
    
    crawl_natenews_page(url, output_dir=output_dir)
